        ...
        <div style={S.content}>
          ...
          {currentStep === 'type' && (
            <div style={S.typeGrid}>
              {[
                { value: 'standard',    icon: '🐍', label: 'STANDARD',    desc: 'Classic snake draft. Teams pick in order, reversing each round.' },
                { value: 'auction',     icon: '🔨', label: 'AUCTION',     desc: 'All picks are nominated and bid on. Budget management is key.' },
                { value: 'combination', icon: '⚡', label: 'COMBINATION', desc: 'Starts with auction rounds, then finishes with a snake draft.' },
              ].map(({ value, icon, label, desc }) => (
                <div key={value}
                  style={{ ...S.typeCard, ...(draftType === value ? S.typeCardActive : {}) }}
...
const S = {
  ...
  typeGrid: { display: 'flex', flexDirection: 'column', gap: 12 },
  typeCard: {
  ...
  typeLabel: { fontFamily: "'Bebas Neue', sans-serif", fontSize: 20, letterSpacing: 3, color: 'var(--text)', minWidth: 120 },
  ...
  bigInput: {
    ...
    textAlign: 'center', width: 160, padding: '8px 0',
  },
  stepper: {
    background: 'var(--surface2)',
    border: '1px solid var(--border)',
